...
name: slack
description: |
  Complete Slack integration for Cursor workflow. Enables full Slack access without leaving your editor:
  - Check unread messages across all workspaces
  - Read channel history and DMs
  - Search conversations (full-text with filters)
  - Set status and DND mode
  - View activity feed (mentions, reactions, threads)
  - Send messages to channels and DMs

  Use when user says:
  - "/slack" (shows unread messages)
  - "check slack", "read slack messages"
  - "search slack for X"
  - "set my slack status"
  - "send slack message"
  ...
  - Fast CLI-based access (no browser loading)
