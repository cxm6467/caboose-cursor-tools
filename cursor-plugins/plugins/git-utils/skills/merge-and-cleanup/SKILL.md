...
description: |
  Complete PR merge workflow with full cleanup automation. Performs the entire post-review process:
  1. Validates PR is ready to merge (checks status, CI, conflicts)
  2. Squash-merges the PR via GitHub
  3. Deletes the feature branch (local and remote)
  4. Switches back to default branch (main/master)
  5. Pulls latest changes
  6. Cleans up worktrees if applicable
  7. Provides summary of all actions taken
  ...
  - Auto-detects default branch (main, master, or custom)
  - Provides clear error messages and recovery commands

  BENEFIT: Eliminates 5+ manual git commands into one skill call, with error handling and edge case detection.
